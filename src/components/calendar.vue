  ...
  <div class="calendar">
    ...
    <div class="weekHeader">
      <span v-for="(week,index) in weeks"
            class="week">
        <span :class="[(week=='星期六'||week=='星期日')?'vacation':'']">{{ week }}</span>
      </span>
    </div>
    <div class="calendarDate">
      <div v-for="(day,index) in days"
           :key="index"
           class="rowDays">
        <span v-for="(item,i) in days[index]"
              class="day">
          <span v-if="item.day"
                :class="[(i==5||i==6)?'vacation':'',item.day===now?'active':'']"
                @click="changeDay(item.day)">{{ item.day }}</span>
        </span>
      </div>
    </div>
  </div>
...
export default {
  data () {
    return {
      ...
      days: [[{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }],//一共七行,每行显示的号数
      [{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }],
      [{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }],
      [{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }],
      [{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }],
      [{ day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }, { day: 0, month: 0 }]]
  ...
  methods: {
    ...
    preMonth () {
      this.currentDay = new Date(this.currentDay.setMonth(this.currentDay.getMonth() - 1))
    ...
    is_leap (year) {//判断是否是闰年(true为闰年，false不为闰年)
      // 闰年的条件是符合下面二者之一：
      // (1)年份能被4整除，但不能被100整除；
      // (2)年份能被400整除。
    ...
    formatDays (date, preOrnext) {//对日历数据的处理，传入一个日期
      if (preOrnext) {
        this.now = false
      } else {
      ...
      if (this.currentMonth == 2) {//获取天数
        this.currentMonthDays = this.is_leap(date.getFullYear())
      } else {
        this.currentMonthDays = new Date(this.currentYear, this.currentMonth, 0).getDate()
      }
      ...
      day += 1;
      for (let i = 0; i < this.currentMonthWeek - 1; i++) {//给days第一行数据前面几条赋值
        this.days[0][i]["day"] = 0;
        this.days[0][i]["month"] = this.nowMonth;
      }
      for (let i = this.currentMonthWeek; i < this.days[0].length; i++) {//给days第一行数据后面几条赋值
        this.days[0][i]["day"] = day;
        this.days[0][i]["month"] = this.nowMonth;
        day += 1;
      }
      for (let i = 1; i < this.days.length; i++) {//遍历除去days的第一条数据,从后面的开始遍历赋值
